...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-[500px] w-full bg-goat-bg text-goat-accent">
      <AppHeader audioEngine={audioEngine} />
      <TabNavigation activeTab={activeTab} setActiveTab={setActiveTab} />
      <TabContent activeTab={activeTab} audioEngine={audioEngine} />
    </div>
  );
}

function AppHeader({ audioEngine }) {
  return (
...
function TabContent({ activeTab, audioEngine }) {
  return (
    <div className="flex-1 relative overflow-hidden bg-goat-bg">
      <AnimatePresence mode="wait">
        {activeTab === 'controls' && <ControlsPanel audioEngine={audioEngine} />}
...
function ControlsPanel({ audioEngine }) {
  return (
    <motion.div
      key="controls"
    ...
    >
      ...
      <div className="flex gap-2 mt-4">
        <button onClick={audioEngine.resetFilters} className="text-xs px-2 py-1 border border-goat-accent rounded hover:bg-goat-accent/10">
          Reset Filters
        </button>
      </div>
    </motion.div>
